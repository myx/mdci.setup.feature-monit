...
- So on `mel` hosts the wrapper is deployed into `/usr/local/etc/mdci-monit-scripts/` and called by nothing. The fix is correct and its blast radius here is zero. Rules in other namespaces are not deployed from this workspace and are not assessed here.
...
- **The open question, and it is not this file's to answer.** Which source tree populates `/usr/local/etc/mdci-monit-scripts/` on the hosts running those pullers. If it is not this workspace, the live defect is still open on them and this change did not touch it. `keeper-ndm` owns that estate and is the one who can settle it. **Nothing here asserts an answer, and no session should read one into this entry.**
